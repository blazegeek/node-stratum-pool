...
exports.CreateGeneration = function(rpcData, publicKey, extraNoncePlaceholder, reward, txMessages, recipients, network){
	...
	var txComment = txMessages === true ?
		util.serializeString('https://github.com/blazegeek/node-stratum-pool') :
		new Buffer([]);
	...
	var scriptSigPart2 = util.serializeString('/blazepool/');
	...
	/*
	The generation transaction must be split at the extranonce (which located in the transaction input scriptSig).
	Miners send us unique extranonces that we use to join the two parts in attempt to create a valid share and/or block.
	*/
